...
export const UserMenu: React.FC<Props> = ({ profile, lang, onNavigate, onLogout }) => {
  ...
  // Simple formatter to clean up Plan IDs if they are raw strings
  const formatPlanName = (plan: string) => {
    if (plan.startsWith('PLAN_')) return isRtl ? 'باقة مخصصة' : 'Custom Plan';
    return plan;
  };
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        <div className="w-10 h-10 bg-slate-900 rounded-full flex items-center justify-center text-white font-black text-sm group-hover:bg-blue-600 transition-colors shadow-sm">
        ...
        <div className="text-start hidden sm:block">
          ...
          <p className="text-xs font-bold text-slate-700 max-w-[120px] truncate">
      ...
      {isOpen && (
        <div className={`absolute top-full mt-3 w-64 bg-white rounded-[2rem] border border-slate-100 shadow-2xl z-[100] p-2 animate-in fade-in slide-in-from-top-2 ${isRtl ? 'left-0' : 'right-0'}`}>
          ...
          <div className="p-5 bg-slate-50/50 border-b border-slate-100 rounded-t-[1.5rem] mb-2 text-center">
            <h4 className="text-sm font-black text-slate-900 truncate px-2">
              {profile.displayName || profile.email.split('@')[0]}
            </h4>
            <div className="flex items-center justify-center gap-1.5 mt-1.5">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></span>
              <span className="text-[10px] font-black text-blue-600 uppercase tracking-wider">
                {isRtl ? 'باقة' : 'Plan'}: {formatPlanName(profile.plan)}
              </span>
            </div>
            <p className="text-[9px] font-bold text-slate-400 mt-2 truncate opacity-60">{profile.email}</p>
          </div>
